...
exports.getSessionContext = async (contextId) => {
  ...
  return {
    ...
    interviewMessageId: lastMessage ? lastMessage.id : null,
    messages: messages,
    // Flat fields are consumed by the existing Gemini interview service.
    company: context.company,
    role: context.role,
    difficulty: context.difficulty,
    interviewType: context.type,
    durationMinutes: context.duration_minutes,
    candidateSummary: context.candidate_summary,
    skills: context.candidate_summary?.skills || [],
    projects: context.candidateProfile?.projects || [],
    resumeSummary: context.candidateProfile?.summary || '',
    yearsOfExperience: context.candidate_summary?.totalYearsExperience || 0,
    candidate: {
